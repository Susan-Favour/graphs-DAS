#Accessing an element in a graph
class Graph:
    def __init__(self):
        self.vertices = {} 
#empty dictionary vertices within the graph object to store vertices as keys
...
#undirected graph - is a type of graph in which edges have a direction associated with them.
class Graph:
    def __init__(self, size):
        self.adj_matrix = [[None] * size for _ in range(size)]
#represents the connections between vertices in the graph.
        self.size = size
        self.vertex_data = [''] * size
    def add_edge(self, u, v, weight):
        if 0 <= u < self.size and 0 <= v < self.size:
#Checks if the given vertices u and v are within the valid range of vertex indices.
...
#Weighted Graph - each edge has an associated numerical value, called a weight or cost.
class Graph:
     def __init__(self, size):
